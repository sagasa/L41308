...
　　　　public int TitleCount = 0;//タイトルの状態管理の値。0がタイトル、1がステージセレクト画面です。
        ...
        public override void Draw()
        {
            ...
            if (TitleCount==1)//ステージセレクト
            {
                ...
                for (int i = 1; i <= 3; i++)
                {
                    NumberDraw.ScoreDraw(Game.hightScore.scoreRankings[i][0].score, Screen.Width * i + 280 - (int)BackgroundFixPosition, 468, fontInterval, fontScale1, "image_select/mozi_");
                    NumberDraw.TimeDraw(DateTime.FromBinary(Game.hightScore.timeRankings[i][0].timeBinary), Screen.Width * i + 280 - (int)BackgroundFixPosition, 532, fontInterval, fontScale1, "image_select/mozi2_");
                }
                Dummy.Draw();
            }
        }
