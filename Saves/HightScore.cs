...
namespace Giraffe.Saves
{
    public class HightScore
    {
        ...
        public void RankingSort(Entry data, int stageNum)
        {
            //10番を追加、データ代入
            scoreRankings[stageNum].Add(new Entry());
            timeRankings[stageNum].Add(new Entry());
            scoreRankings[stageNum][10] = data;
            timeRankings[stageNum][10] = data;
            //ソート
            for (int i = 0; i < 10; i++)
            {
                //スコアのソート
                ...
                if (timeRankings[stageNum][i].timeBinary <= scoreRankings[stageNum][10].timeBinary)
                {
